...
class ClubInfoServiceTest {
    ...
    void shouldReturnClubInfo_whenClubExists() {
        ...
        given(clubRepository.findByClubId(1L)).willReturn(Optional.of(club));

        // When
        ClubInfoResponse response = clubInfoService.getClubInfo(1L);
    ...
    void shouldThrowException_whenClubNotFound() {
        // Given
        given(clubRepository.findByClubId(99L)).willReturn(Optional.empty());

        // When / Then
        assertThrows(ClubNotFoundException.class, () -> clubInfoService.getClubInfo(99L));
    }
}
